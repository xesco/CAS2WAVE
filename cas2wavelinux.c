...
int main(int argc, char *argv[]) {
    ...
    unsigned char *b0, *b1;
    int casfd; // CAS file descriptor
    int wavfd; // WAV file descriptor
    ...
    casfd = open(argv[casfile], O_RDONLY);
    // TODO errno
    //if (casfile < 0) {
    ...
    if (wavfile) {
        wavfd = open(argv[wavfile], O_WRONLY | O_CREAT | O_TRUNC);
    }
    //TODO errno
    //if (wavfile < 0) {
    ...
    if (write(wavfd, "RIFF    WAVEfmt \20\0\0\0\1\0\1\0\104\254\0\0\104\254\0\0\1\0\10\0data    ",0x2c) < 0x2c) {
        puts("Can't write output file");
        close(wavfd);
        exit(0);
    }

    //while (n=read(casfd,buffer,8)) {
